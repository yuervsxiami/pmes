...
(function (angular) {
  ...
  angular.module('Statistics')
      .controller('CollegeStatCtrl', ['$scope', '$state', 'ToastService', 'ChartService', 'DEFAULT_PROVINCE_COLLEGE', CollegeStatCtrl])
			.controller('CollegePmesValuePatentCtrl', ['$scope', '$state', 'ToastService', 'ChartService', 'PatentService', CollegePmesValuePatentCtrl])
    ;

  function CollegeStatCtrl($scope, $state, ToastSerrvice, ChartService, DEFAULT_PROVINCE_COLLEGE) {
		...
		self.init = function () {
			// 初始化的省份名称
			self.initParam = DEFAULT_PROVINCE_COLLEGE;
		...
		self.getProvinces = function () {
			ChartService.getProvinces().then(function(resp) {
				if (resp && resp.code === 0) {
					...
					if(self.initParam.provinceName && !self.hasFirstInit) {
		...
		self.getColleges = function() {
			...
			ChartService.getColleges(self.searchCondition.provinceName).then(function(resp) {
				if (resp && resp.code === 0) {
					...
					if(self.initParam.collegeName && !self.hasFirstInitCollege) {
	...
	function CollegePmesValuePatentCtrl($scope, $state, ToastService, ChartService, PatentService) {
